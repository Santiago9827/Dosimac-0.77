...
export const db = SQLite.openDatabase({ name: 'farmdb.db', location: 'default' });

export const InicialiceFarmDataTable = () => new Promise<void>((resolve, reject) => {
    db.transaction(tx => {
        tx.executeSql(
            `CREATE TABLE IF NOT EXISTS farms(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT, location TEXT, province TEXT,
        userName TEXT, password TEXT, ssid TEXT,
        wifiPassword TEXT, serverIp TEXT
      )`,
            [],
            () => resolve(),
            (_tx, err) => { console.log('SQL CREATE ERR', err); reject(err); return true; }
        );
    });
});
