...
export const AWRScanResultsScreen = ({ navigation }) => {
    ...
    const handleNoDevicesAccept = () => {
        ...
        else navigation.navigate('AWR-STARTSCAN' as never); // por si entraron directo
    };
    ...
    const macHasAllowedPrefix = (id?: string | null) => {
        ...
        if (parts.length >= 3) {
            ...
            return ['00:04:3E'].includes(oui); // OUI de tu captura
        }
        return false;
    };
    ...
    const isAWR = (d: BlePeripheral): boolean => {
        const local = getLocalName(d).toUpperCase();
    ...
    const labelFor = (d: BlePeripheral) => getLocalName(d) || (d.id ?? '').toUpperCase();
    ...
    const renderDevice = (device: BlePeripheral) => {
        ...
        return (
            <View key={device.id} style={{ marginTop: 12 }}>
                <MainButton
                    onPress={() => {
                        // De momento solo listamos; en la siguiente iteración conectamos y suscribimos.
