...
export default function TodosAnimalesMaternidad() {
    ...
    const dataSorted = useMemo(() => {
        ...
        if (sort.key === 'pct') {
            copy.sort(
                (a, b) => dir * (pct(a.consumida, a.total) - pct(b.consumida, b.total)),
            );
            ...
            copy.sort(
                (a, b) =>
                    dir * a.corral.localeCompare(b.corral, 'es', { numeric: true }),
            );
        }
        return copy;
    }, [data, sort]);

    // ── NAVEGAR AL DETALLE (igual que en NoAlimentados) ──────────────────────
    ...
    return (
        <SafeAreaView edges={['bottom']} style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
            ...
            <View
                onLayout={(e) =>
                    setHeaderBox({ y: e.nativeEvent.layout.y, h: e.nativeEvent.layout.height })
                }
                style={{
