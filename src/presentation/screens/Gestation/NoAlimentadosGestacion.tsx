...
type Animal = {
    ...
    consumida: number;
    nota?: string;
    diasSinAlimentar?: number; // 👈 añadido
};
...
export default function NoAlimentadosGestacion() {
    ...
    const openAnimal = (item: Animal) => {
        ...
        navigation.navigate('GET-ANIMAL-DETAIL', {                 // ← usa el name que registres
    ...
    const dataSorted = useMemo(() => {
        ...
        if (sort.key === 'dias') {
            copy.sort((a, b) => {
                ...
                return pct(a.consumida, a.total) - pct(b.consumida, b.total); // empate: menos % primero
            });
    ...
    const Item = ({ item }: { item: Animal }) => (
        <TouchableOpacity activeOpacity={0.85} onPress={() => openAnimal(item)}>
            ...
            >
                ...
                {/* Consumo + chip de días a la derecha */}
                <View className="mt-3 rounded-xl border p-3" style={{ backgroundColor: '#F8FAFC', borderColor: '#E2E8F0' }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
                        ...
                        {typeof item.diasSinAlimentar === 'number' && (() => {
                            ...
                            return (
                                <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999, backgroundColor: bg }}>
        ...
        <View className="flex-1 bg-slate-50">
            ...
            >
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Text className="ml-2 text-slate-900 text-[18px] font-extrabold">No alimentados · Gestación</Text>
                </View>
            ...
            {/* Chip del orden activo (sin X en el default) */}
            <View style={{ paddingHorizontal: 16, marginBottom: 8 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    >
                        ...
                        <Text style={{ color: BRAND, fontWeight: '800', marginLeft: 6 }}>{ORDER_LABEL[sort.key]}</Text>
                    </TouchableOpacity>
            ...
            {/* Menú de orden anclado al botón */}
            <Modal
                visible={menuOpen}
                transparent
                animationType="fade"
                        ...
                        return (
                            <View
                                style={{
                                    ...
                                    borderRadius: 12,
                                    shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 12, shadowOffset: { width: 0, height: 6 }, elevation: 12,
                                    overflow: 'hidden',
                                }}
                            >
